...
namespace ManualMapInjection.Injection
{
    internal class ManualMapInjector
    {
        ...
        private IntPtr RvaToPointer(uint rva, IntPtr baseAddress)
        {
            ...
            return Imports.ImageRvaToVa(
                imageNtHeaders.Address,
                baseAddress,
                new UIntPtr(rva),
                IntPtr.Zero
            );
        ...
        private bool InjectDependency(string dependency)
        {
            ...
            var procAddress = Imports.GetProcAddress(
                Imports.GetModuleHandle("kernel32.dll"),
                "LoadLibraryA"
            );
            ...
            var result = Imports.WriteProcessMemory(
                _hProcess,
                lpAddress,
                buffer,
                buffer.Length,
            ...
            if (result)
            {
                var hHandle = Imports.CreateRemoteThread(
                    _hProcess,
                    IntPtr.Zero,
                    0,
                    procAddress,
        ...
        private IntPtr GetRemoteModuleHandleA(string module)
        {
            ...
            var pbi = (PPROCESS_BASIC_INFORMATION)Imports.HeapAlloc(
            ...
            uint dwSizeNeeded;
            var dwStatus = Imports.NtQueryInformationProcess(
                _hProcess, /*ProcessBasicInformation*/
                0,
                pbi.Address,
                dwSize,
                out dwSizeNeeded
            ...
            if (dwStatus >= 0 && dwSize < dwSizeNeeded)
            {
                ...
                pbi = (PPROCESS_BASIC_INFORMATION)Imports.HeapAlloc(
                    hHeap, /*HEAP_ZERO_MEMORY*/
                    0x8,
                    new UIntPtr(dwSize)
                );
                ...
                dwStatus = Imports.NtQueryInformationProcess(
                    _hProcess, /*ProcessBasicInformation*/
                    0,
                    pbi.Address,
                    dwSizeNeeded,
            ...
            if (dwStatus >= 0)
            {
                if (pbi.Value.PebBaseAddress != IntPtr.Zero)
                {
                    ...
                    if (
                        Imports.ReadProcessMemory(
                            _hProcess,
                            pbi.Value.PebBaseAddress
                                + 12 /*peb.Ldr*/
                            ,
                            out pebLdrAddress,
                            out dwBytesRead
        ...
        private IntPtr GetDependencyProcAddressA(IntPtr moduleBase, PCHAR procName)
        {
            ...
            if (expBase > 0)
            {
                ...
                Imports.ReadProcessMemory(
                    _hProcess,
                    moduleBase + (int)expBase,
                    expData.Address,
                    (int)expSize,
                    out dwRead
                );

                var pAddressOfOrds = (PWORD)(
                    expData.Address + (int)expData.Value.AddressOfNameOrdinals - (int)expBase
                );
                var pAddressOfNames = (PDWORD)(
                    expData.Address + (int)expData.Value.AddressOfNames - (int)expBase
                );
                var pAddressOfFuncs = (PDWORD)(
                    expData.Address + (int)expData.Value.AddressOfFunctions - (int)expBase
                );
                ...
                for (uint i = 0; i < expData.Value.NumberOfFunctions; i++)
                {
                    ...
                    if (new PDWORD(procName.Address).Value <= 0xFFFF)
                    {
                        ...
                        new PDWORD(procName.Address).Value > 0xFFFF
                        && i < expData.Value.NumberOfNames
                    )
                    {
                        pName = (PCHAR)new IntPtr(
                            pAddressOfNames[i] + expData.Address.ToInt32() - expBase
                        );
                    ...
                    if (
                        ...
                        || (
                            new PDWORD(procName.Address).Value > 0xFFFF
                            && pName.ToString() == procName.ToString()
                        )
                    )
                    {
                        ...
                        if (
                            pFunc.ToInt64() >= (moduleBase + (int)expBase).ToInt64()
                            && pFunc.ToInt64()
                                <= (moduleBase + (int)expBase + (int)expSize).ToInt64()
                        )
                        {
                            ...
                            {
                                pFunc = GetDependencyProcAddressA(
                                    hChainMod,
                                    new PCHAR(strName) + 1
                                );
                            }
        ...
        private bool ProcessImportTable(IntPtr baseAddress)
        {
            ...
            {
                var imageImportDescriptor = (PIMAGE_IMPORT_DESCRIPTOR)RvaToPointer(
                    imageNtHeaders.Value.OptionalHeader.ImportTable.VirtualAddress,
                    baseAddress
                );
                ...
                if (imageImportDescriptor != null)
                {
                    ...
                    {
                        var moduleName = (PCHAR)RvaToPointer(
                            imageImportDescriptor.Value.Name,
                            baseAddress
                        );
                        if (moduleName == null)
                        {
...
#if DEBUG
                                Debug.WriteLine(
                                    "[ProcessImportTable] failed to obtain module handle"
                                );
#endif
                        ...
                        {
                            imageThunkData = (PIMAGE_THUNK_DATA)RvaToPointer(
                                imageImportDescriptor.Value.OriginalFirstThunk,
                                baseAddress
                            );
                            imageFuncData = (PIMAGE_THUNK_DATA)RvaToPointer(
                                imageImportDescriptor.Value.FirstThunk,
                                baseAddress
                            );
                        }
                        else
                        ...
                        for (
                            ;
                            imageThunkData.Value.AddressOfData > 0;
                            imageThunkData++, imageFuncData++
                        )
                        {
                            IntPtr functionAddress;
                            var bSnapByOrdinal =
                                (
                                    imageThunkData.Value.Ordinal
                            ...
                            if (bSnapByOrdinal)
                            {
                                ...
                                functionAddress = GetDependencyProcAddressA(
                                    moduleBase,
                                    new PCHAR(ordinal)
                                );
                            ...
                            {
                                var imageImportByName = (PIMAGE_IMPORT_BY_NAME)RvaToPointer(
                                    imageFuncData.Value.Ordinal,
                                    baseAddress
                                );
                                var mameOfImport =
                                    (PCHAR)imageImportByName.Address
...
#if DEBUG
                    Debug.WriteLine(
                        "[ProcessImportTable] Size of table confirmed but pointer to data invalid"
                    );
#endif
        ...
        private bool ProcessDelayedImportTable(IntPtr baseAddress, IntPtr remoteAddress)
        {
            ...
            {
                var imageDelayedImportDescriptor = (PIMAGE_IMPORT_DESCRIPTOR)RvaToPointer(
                    imageNtHeaders.Value.OptionalHeader.DelayImportDescriptor.VirtualAddress,
                    baseAddress
                );
                ...
                if (imageDelayedImportDescriptor != null)
                {
                    for (
                        ;
                        imageDelayedImportDescriptor.Value.Name > 0;
                        imageDelayedImportDescriptor++
                    )
                    {
                        var moduleName = (PCHAR)RvaToPointer(
                            imageDelayedImportDescriptor.Value.Name,
                            baseAddress
                        );
                        if (moduleName == null)
                        {
                        ...
                        {
                            imageThunkData = (PIMAGE_THUNK_DATA)RvaToPointer(
                                imageDelayedImportDescriptor.Value.OriginalFirstThunk,
                                baseAddress
                            );
                            imageFuncData = (PIMAGE_THUNK_DATA)RvaToPointer(
                                imageDelayedImportDescriptor.Value.FirstThunk,
                                baseAddress
                            );
                        }
                        else
...
#if DEBUG
                    Debug.WriteLine(
                        "[ProcessDelayedImportTable] Size of table confirmed but pointer to data invalid"
                    );
#endif
                    // Size of table confirmed but pointer to data invalid
                    return false;
            ...
            switch ((data >> 12) & 0xF)
            {
                case 1: // IMAGE_REL_BASED_HIGH
                    ...
                    Marshal.WriteInt16(
                        raw.Address,
                        unchecked(
                            (short)(raw.Value + (uint)((ushort)((imageBaseDelta >> 16) & 0xffff)))
                        )
                    );
                    break;
                ...
                case 2: // IMAGE_REL_BASED_LOW
                    ...
                    Marshal.WriteInt16(
                        raw.Address,
                        unchecked((short)(raw.Value + (uint)((ushort)(imageBaseDelta & 0xffff))))
                    );
                    break;
        ...
        private bool ProcessRelocations(IntPtr baseAddress, IntPtr remoteAddress)
        {
            ...
            if (
                (
                    imageNtHeaders.Value.FileHeader.Characteristics
            ...
            {
                var imageBaseDelta = (uint)(
                    remoteAddress.ToInt32() - imageNtHeaders.Value.OptionalHeader.ImageBase
                );
                var relocationSize = imageNtHeaders.Value.OptionalHeader.BaseRelocationTable.Size;

                if (relocationSize > 0)
                {
                    var relocationDirectory = (PIMAGE_BASE_RELOCATION)RvaToPointer(
                        imageNtHeaders.Value.OptionalHeader.BaseRelocationTable.VirtualAddress,
                        baseAddress
                    );
                    ...
                    {
                        var relocationEnd =
                            (PBYTE)relocationDirectory.Address + (int)relocationSize;

                        while (
                            relocationDirectory.Address.ToInt64() < relocationEnd.Address.ToInt64()
                        )
                        {
                            var relocBase = (PBYTE)RvaToPointer(
                                relocationDirectory.Value.VirtualAddress,
                                baseAddress
                            );
        ...
        )
        {
            ...
            if (
                !Imports.WriteProcessMemory(
                    _hProcess,
                    new IntPtr(remoteAddress.ToInt64() + (long)virtualAddress),
                    new IntPtr(baseAddress.ToInt64() + (long)rawData),
                    new IntPtr((long)rawSize),
            ...
            if (
                !Imports.VirtualProtectEx(
                    _hProcess,
                    new IntPtr(remoteAddress.ToInt64() + (long)virtualAddress),
                    new UIntPtr(virtualSize),
                    protectFlag,
                    out dwOldProtect
                )
            )
        ...
        private bool ProcessSections(IntPtr baseAddress, IntPtr remoteAddress)
        {
            ...
            var imageSectionHeader = (PIMAGE_SECTION_HEADER)(
                imageNtHeaders.Address
                + /*OptionalHeader*/
                24
                + imageNtHeaders.Value.FileHeader.SizeOfOptionalHeader
            );
            for (ushort i = 0; i < imageNtHeaders.Value.FileHeader.NumberOfSections; i++)
            {
                ...
                if (
                    characteristics.HasFlag(DataSectionFlags.MemoryRead)
                    || characteristics.HasFlag(DataSectionFlags.MemoryWrite)
                    || characteristics.HasFlag(DataSectionFlags.MemoryExecute)
                )
                {
                    ...
                    ProcessSection(
                        ...
                        imageSectionHeader[i].VirtualSize,
                        protection
                    );
                }
            }
                ...
                if (async)
                {
                    var t = new Thread(
                        () =>
                        {
                            ...
                            Imports.VirtualFreeEx(
                                _hProcess,
                                lpAddress,
                                0,
                                Imports.FreeType.Release
                            );
                        }
                    )
                    {
        ...
        private bool CallEntryPoint(IntPtr baseAddress, uint entrypoint, bool async)
        {
            ...
            buffer.Add(0x68);
            buffer.AddRange(
                BitConverter.GetBytes( /*DLL_PROCESS_ATTACH*/
                    1
                )
            );
            buffer.Add(0x68);
        ...
        private bool ProcessTlsEntries(IntPtr baseAddress, IntPtr remoteAddress)
        {
            ...
            var tlsDirectory = (PIMAGE_TLS_DIRECTORY32)RvaToPointer(
                imageNtHeaders.Value.OptionalHeader.TLSTable.VirtualAddress,
                baseAddress
            );
            ...
            if (
                !Imports.ReadProcessMemory(
                    _hProcess,
                    new IntPtr(tlsDirectory.Value.AddressOfCallBacks),
                    buffer,
                    out dwRead
                )
            )
        ...
        private IntPtr LoadImageToMemory(IntPtr baseAddress)
        {
            ...
            for (uint i = 0; i < imageNtHeaders.Value.FileHeader.NumberOfSections; i++)
            {
                ...
                if (
                    imageSectionHeader[i].VirtualAddress + imageSectionHeader[i].VirtualSize
                    > rvaHigh
                )
                {
                    rvaHigh =
...
#if DEBUG
                Debug.WriteLine(
                    "[LoadImageToMemory] This method is not supported for Managed executables"
                );
#endif
            ...
            {
                var dllEntryPoint =
                    allocatedRemoteMemory.ToInt32()
                    + (int)imageNtHeaders.Value.OptionalHeader.AddressOfEntryPoint;
